$(function(){
	...
	Map.prototype = {
		...
		add_countries: function(data, launches = null) {
			var countries = [];
			var i, j, img;
				...
				if(launches != null) {
					for(var j = 0; j < launches.length; j++) {
						if(launches[j].country == countries[i].data.name) {
							...
							//set name of mesh
							imgPlane.name = countries[i].data.name;
							
							// rotate and position the elements
							imgPlane.rotation.x = Math.PI/2;
							imgPlane.translateX(-490);
							imgPlane.translateZ(50);
							imgPlane.translateY(20);
							*/
						}
					}
				}
	...
	function init() {
		...
		$.when(	$.getJSON("vendor/worldMap/data/countries.json") ).then(function(data){ 
			...
			function tick(timestamp) {
				...
				if(worldMap.INTERSECTED) {
					if(worldMap.INTERSECTED.netlaunch != "") {
						$('#country-name').html(worldMap.INTERSECTED.name + " - Net Launch Date: " + worldMap.INTERSECTED.netlaunch);
					} else {
						$('#country-name').html(worldMap.INTERSECTED.name + " - No Scheduled Rocket Launches Found");						
					}
				} else {
			...
			// init RocketLaunchAPI
			rocketLaunchAPI = new RocketLaunchAPI();

			rocketLaunchAPI.getUpcomingLaunches(function(launches) {
				if(launches.length > 0) {
					// setup our map to display the upcoming rocket launches
					worldMap.add_countries(data, launches);
				} else {
